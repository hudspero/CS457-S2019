...
void main() {
	...
	if(uSmooth) { //IF SMOOTH ENABLED
		if(c <= 1) { //FOR s,t VALUES WITHIN THE EQUATION OF AN ELLIPSE
			vec3 rgb = vLightIntensity * mix(vec3(vMCposition.x, (pow(vMCposition.y, 1)), vMCposition.z), vColor.rgb, tl);
			gl_FragColor = vec4(rgb, 1.);}
		else { //FOR s,t VALUES OUTSIDE OF THE ELLIPSE EQUATION
			gl_FragColor = vec4((vLightIntensity * vec3(vMCposition.z, (pow(vMCposition.y, 0.99)), vMCposition.x)), 1.);}
	}
	else {
		if(c <= 1) { //FOR s,t VALUES WITHIN THE EQUATION OF AN ELLIPSE
